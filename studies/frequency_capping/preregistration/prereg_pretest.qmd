...
The two variations of the focal ad are displayed in @fig-stimuli. You can find a feed displaying the abstract ads [here](https://web.archive.org/web/20240621212002/https://ibt-hsg.herokuapp.com/p/qxirdbjm/DICE/C_Feed/3).
...
We run a 2-cell (identified victim vs. abstract numbers) between-subjects design with two ad exposures as a pre-test (i.e., only the second column of @fig-design). We pre-test the experiment as we implemented new features in the DICE software, that measure the height in which each post is resolved on a participant's device, for instance. In addition, we measure whether ads are clicked. Most importantly, we also changed the mechanisms randomizing the conditions as well as potential randomizations of the order in which posts of a feed are displayed.
...
We recruit 100 participants in a first pilot. To this end, we create a database containing 200 rows.
